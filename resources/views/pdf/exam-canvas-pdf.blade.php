            ...
            @foreach($items as $item)
                ...
                @php
                    ...
                    if (isset($item['styles'])) {
                        ...
                        $styleString .= "padding: " . ($s['padding'] ?? 0) . "px;";
                ...
                <div class="element" style="{{ $positionStyle }} {{ $styleString }}">
                    ...
                    @elseif($item['type'] == 'student_info')
                        <div class="grid grid-cols-2 gap-2 p-2 h-full w-full items-center text-sm">
                            <div><span class="font-bold">{{ $item['content']['label1'] ?? 'Adı Soyadı:' }}</span>
                                {{ $item['content']['val1'] }}</div>
                            <div><span class="font-bold">{{ $item['content']['label2'] ?? 'Numara:' }}</span>
                                {{ $item['content']['val2'] }}</div>
                            <div><span class="font-bold">{{ $item['content']['label3'] ?? 'İmza:' }}</span>
                                {{ $item['content']['val3'] }}</div>
                            <div><span class="font-bold">{{ $item['content']['label4'] ?? 'Puan:' }}</span>
                                {{ $item['content']['val4'] }}</div>
                        </div>
                    ...
                    @elseif($item['type'] == 'fill_in_blanks')
                        ...
                        @php $questionCounter++; @endphp
                        {{-- YENİ: SADECE KOD BLOKLARINA ÖZEL TASARIM --}}
                    @elseif($item['type'] == 'code')
                        <div class="w-full h-full"
                            style="white-space: pre-wrap; padding: 15px; box-sizing: border-box; font-family: monospace; text-align: left; align-items: flex-start;">
                            {{ is_string($item['content']) ? $item['content'] : ($item['content']['text'] ?? '') }}
                        </div>

                        {{-- ORİJİNAL NORMAL METİN (SADECE nl2br EKLENDİ Kİ LİSTELER ALT ALTA İNSİN) --}}
                    @else
                        <div
                            class="w-full h-full flex items-center {{ isset($item['styles']['textAlign']) && $item['styles']['textAlign'] == 'center' ? 'justify-center' : 'justify-start' }}">
                            @php
                                $textContent = is_string($item['content']) ? $item['content'] : ($item['content']['text'] ?? '');
                            @endphp
                            {!! nl2br(e($textContent)) !!}
                        </div>
                    @endif
